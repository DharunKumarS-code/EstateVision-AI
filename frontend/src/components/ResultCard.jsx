...
function MarketTrend({ price }) {
  ...
  return (
    <div
      className={`flex items-center justify-between px-5 py-4 rounded-xl border ${border} ${bg}`}
    >
...
export default function ResultCard({
  ...
  useEffect(() => {
    ...
    const increment = price / (duration / interval);
  ...
  const confidence = Math.min(
    ...
    Math.max(
      72,
      Math.round(85 - Math.abs(price - 80) * 0.05)
    )
  );
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={tag}
                  className="px-3 py-1.5 rounded-full bg-slate-800 border border-slate-700 text-slate-300 text-sm"
                >
                  {tag}
                </span>
              ))}
